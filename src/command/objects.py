...
class EntityManager:
    ...
    def _check_ball_racket_collision(self, ball: BallEntity, racket: RacketEntity):
        ...
        # === SPIN CALCULATION ===
        # Spin is generated by racket motion tangent to the surface
        # Rubbing upward = topspin, rubbing downward = backspin
        rubber_type = rubber.rubber_type

        # Calculate racket's tangent velocity (velocity component parallel to surface)
        ...
        # Spin axis is perpendicular to both surface normal and tangent velocity
        # This creates spin in the direction of the rubbing motion
        if racket_tangent_speed > 0.1:
            spin_axis = np.cross(surface_normal, racket_tangent_vel)
            spin_axis_norm = np.linalg.norm(spin_axis)
            if spin_axis_norm > 0:
                spin_axis = spin_axis / spin_axis_norm

                # Spin intensity depends on friction and tangent speed
                if rubber_type == RubberType.LONG_PIMPLES:
                    ...
                    ball.spin = ball.spin + spin_axis * racket_tangent_speed * friction * 10
                elif rubber_type == RubberType.PIMPLES:
                    # Short pimples: moderate spin generation
                    ball.spin = ball.spin * 0.5
                    ball.spin = ball.spin + spin_axis * racket_tangent_speed * friction * 25
                elif rubber_type == RubberType.ANTI:
                    # Anti-spin: minimal spin transfer
                    ball.spin = ball.spin * 0.2
                    ball.spin = ball.spin + spin_axis * racket_tangent_speed * friction * 5
                else:
                    # Inverted (normal rubber): high spin generation
                    ball.spin = ball.spin * 0.3
                    ball.spin = ball.spin + spin_axis * racket_tangent_speed * friction * 40
        else:
            # Racket not moving tangentially - just reduce existing spin
            if rubber_type == RubberType.LONG_PIMPLES:
                ball.spin = ball.spin * (-0.5)  # Reverse
            elif rubber_type == RubberType.ANTI:
                ball.spin = ball.spin * 0.2
            else:
                ball.spin = ball.spin * 0.5
